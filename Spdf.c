...
char *fixFilePath(char *nameOfFile)
{
    ...
    //getting the locataion of .
    char *StartPointer = strchr(nameOfFile, '.');
    // getting the extension position e.g 107 -100 
    int extensionPosition = StartPointer - nameOfFile;
    // only copying the result from beginning until 3 location after the . 
...
// Method to  resolve the destination path for the pdf srver to store the files.
char *resolveDestinationPath(char *destinationFilePath, char *nameOfFile, int isDestinationDir)
{
    ...
    // getting the working dirrectory
    if (getcwd(userWorkingDirectory, sizeof(userWorkingDirectory)) != NULL)
    {
        ...
        // if destinationFilePath ~smain/Folder1/Folder2 then filePathAfterRemovingSmain will store /Folder1/Folder2
        strncpy(filePathAfterRemovingSmain, start + length, sizeof(filePathAfterRemovingSmain) - 1);

        if (filePathAfterRemovingSmain != '\0')
        {
           // directoryOfFile will contain /user/kunwar2 + /spdf + /Folder1/Folder2 => /user/kunwar2/spdf/Folder1/Folder2/
            snprintf(directoryOfFile, sizeof(directoryOfFile), "%s/%s%s/", userWorkingDirectory, "spdf", filePathAfterRemovingSmain);
            if (!isDestinationDir)
                snprintf(directoryOfFile, sizeof(directoryOfFile), "%s/%s%s", userWorkingDirectory, "spdf", filePathAfterRemovingSmain);
        }
        else
        ...
        // complete path will be  directoryOfFile + file name
...
void handleCreateandDownloadTARFiles(char *userCommand, char *fileType, int clientSocketReference)
{
    ...
    {
        snprintf(sourceDirectory, sizeof(sourceDirectory), "%s/%s", currentWorkingDir, "spdf");
        snprintf(tarFileLocation, sizeof(tarFileLocation), "%s/%s", currentWorkingDir, tarFileName);
    }
    createTarFile(sourceDirectory, tarFileLocation, "pdf");
}
        ...
        else if (S_ISREG(path_stat.st_mode))
        {
            if (doesContainProvidedExtension(entry->d_name, fileExtension))
            {
                ...
                if (strstr(directoryPathName, "smain/") != NULL)
                    sourceDirectoryPathName = convertPathNameFromSmainToSTextServer(directoryPathName, "spdf/");
                else
                    sourceDirectoryPathName = convertPathNameFromSmainToSTextServer(directoryPathName, "spdf");
...
// Refer line no 372 in Smain.c
void handleDisplayFiles(char *userCommand, char *directoryPath, int clientSocketReference)
{
    char *completeDirectoryPath = resolveDirectoryPath(directoryPath, 1);
    createSubDirectoriesAndFiles(completeDirectoryPath, "pdf");
}

//Method to resolve the directory path by removing the ~smain
//Refer line no 541 in Smain.c
char *resolveDirectoryPath(char *destinationFilePath, int isDestinationDir)
{
    char userWorkingDirectory[1024];
...
// This method is purly for the Spdf server to send the requested file to the Smain which he/she has requested to download form smain
//Refer line no 493 in Smain.c
void sendDownloadedFileFromPDFToClient(char *sourceFile, int clientSocketReference)
{
    char *nameOfFile = basename(sourceFile);
